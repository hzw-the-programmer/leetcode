...
fn t3() {
    let tests = [
        ("".to_string(), btree![]),
        ("1,,".to_string(), btree![1]),
        ("1,2,,,".to_string(), btree![1, 2]),
        ("1,2,,,3,,".to_string(), btree![1, 2, 3]),
        ("1,,2,,".to_string(), btree![1, null, 2]),
        ("1,,2,3,,,".to_string().to_string(), btree![1, null, 2, 3]),
        (
            "1,2,,,3,4,,,5,,".to_string(),
            btree![1, 2, 3, null, null, 4, 5],
        ),
    ...
    let codec = Codec::new();
    for (i, test) in tests.iter().cloned().enumerate() {
        assert_eq!(codec.deserialize(test.0), test.1, "{} failed", i);
    }
}
